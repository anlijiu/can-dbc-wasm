...
pub fn parse(dbc_in: &str) -> JsValue {
    ...
    match DBC::try_from(dbc_in) {
        ...
        Err(e) => {
            match e {
                Error::Nom(nom::Err::Error(e)) => eprintln!("{:?}", e),
                Error::Nom(nom::Err::Failure(e)) => eprintln!("{:?}", e),
                Error::Nom(nom::Err::Incomplete(needed)) => eprintln!("Nom incomplete needed: {:#?}", needed),
                Error::Incomplete(dbc, remaining) => eprintln!("Not all data in buffer was read {:#?}, remaining unparsed (length: {}): {}\n...(truncated)", dbc, remaining.len(), remaining),
                Error::MultipleMultiplexors => eprintln!("Multiple multiplexors defined"),
            }
        }
    }
